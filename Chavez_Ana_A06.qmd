...
bears_value <- which.max(spread)
cat("The largest win spread is", max(spread), "when the Bears scored", bears[bears_value] , "points and the Opponent scored", opponent[bears_value], "points.")
```
...
```{r}
game_score <- matrix(c(bears, opponent), nrow = length(bears), ncol = 2 )
...
```{r}
new_scores <- matrix(c(37, 9, 17, 17), nrow = 2, ncol = 2, byrow = TRUE)
...
```{r}
first_win <- which(game_score[,3] == 1) [1]
lost_games <- (first_win - 1)

cat("The Bears lost", lost_games, "games before their first win.")
```
...
cat("The number of factors is:" , number_of_factors(8))
```
...
area_rectangle <- function(length = 48, width = 48) {
  sqin <- length * width
  sqft <- sqin/(12^2)
  return(list(sqin = sqin, sqft = sqft))
}

result <- area_rectangle(102, 60)

cat("The area in square inches is", result$sqin, "\n")
  cat("The area in square feet is", result$sqft)
```
...
most_letters <- function(string) {
  max_length <- max(nchar(string))
  return(string[nchar(string) == max_length])
}
...
```{r}
is_in_vector <- function(value, vector) {
  return(value %in% vector)
}


if (is_in_vector(value, check)) {
...
```{r}
negative_matrix <- function(matrix) {
